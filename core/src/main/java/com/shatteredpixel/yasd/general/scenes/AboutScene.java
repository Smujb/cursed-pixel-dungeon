...
public class AboutScene extends PixelScene {
	...
	public void create() {
		...
		//*** Cursed PD Credits ***
		CreditsBlock CPD = new CreditsBlock(true, Window.SHPX_COLOR,
				"Cursed Pixel Dungeon",
				Icons.YENDOR.get(),
				"Developed by: _Samuel Braithwaite_\nBased off Yet Another Shattered Dungeon (wait, it's the same guy?)",
				"Discord Server",
				DISCORD_LINK);
		CPD.setRect((w - fullWidth)/2f, 6, 120, 0);
		content.add(CPD);
		...
		CreditsBlock YASD = new CreditsBlock(true, Window.SHPX_COLOR,
				...
				"Discord Server",
				DISCORD_LINK);
		YASD.setRect((w - fullWidth)/2f, CPD.bottom() + 8, 120, 0);
		content.add(YASD);

		addLine(YASD.top() - 4, content);
